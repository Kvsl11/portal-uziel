...
const LyricsToolbar = ({ textareaRef, onInsert }: { textareaRef: React.RefObject<HTMLTextAreaElement>, onInsert: (val: string) => void }) => {
    ...
    return (
        <div className="flex flex-col gap-3 p-3 md:p-4 bg-white dark:bg-[#0b1221] rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm mx-2 md:mx-4 -mt-4 relative z-20">
            <div className="flex items-center gap-2 md:gap-3 overflow-x-auto no-scrollbar pb-1">
                <button type="button" onClick={() => insertTag('**', '**')} className="w-8 h-8 md:w-9 md:h-9 rounded-xl bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 flex items-center justify-center font-extrabold text-slate-700 dark:text-slate-200 transition-colors shadow-sm shrink-0" title="Negrito">B</button>
                <button type="button" onClick={() => insertTag('*', '*')} className="w-8 h-8 md:w-9 md:h-9 rounded-xl bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 flex items-center justify-center italic font-serif text-slate-700 dark:text-slate-200 transition-colors shadow-sm shrink-0" title="Itálico">I</button>
                <div className="w-px h-6 bg-slate-200 dark:bg-white/10 mx-0.5 shrink-0"></div>
                <div className="flex items-center gap-2 md:gap-3 py-1 px-1 relative flex-1 min-w-0">
                    {palette.map((c: any, index: number) => (
                        <div key={index} className="relative group shrink-0">
                            <button type="button" onClick={() => insertTag(`<c:${c.hex}>`, `</c>`)} onContextMenu={(e) => { e.preventDefault(); setEditingSlot(index); }} className="w-7 h-7 md:w-8 md:h-8 rounded-full transition-transform hover:scale-110 relative shrink-0 shadow-sm ring-2 ring-white dark:ring-[#0b1221] cursor-pointer" style={{ backgroundColor: c.hex }} title="Clique para aplicar. Botão direito para editar."></button>
                    ...
                    <div className="relative group ml-1 shrink-0">
                        <label htmlFor="custom-color-picker" className="cursor-pointer w-7 h-7 md:w-8 md:h-8 rounded-full border-2 border-dashed border-slate-300 dark:border-slate-600 flex items-center justify-center text-slate-400 hover:border-brand-500 hover:text-brand-500 transition-colors bg-transparent"><i className="fas fa-plus text-[10px]"></i></label>
                ...
                <button 
                    ...
                    className="px-2 md:px-3 py-1.5 rounded-lg bg-red-50 dark:bg-red-500/10 hover:bg-red-100 dark:hover:bg-red-500/20 text-red-500 text-[9px] md:text-[10px] font-bold uppercase tracking-widest transition-colors flex items-center gap-1.5 md:gap-2 shrink-0" 
                    title="Remove todas as cifras e marcadores de seção da letra"
                >
                    <i className="fas fa-eraser"></i> <span className="hidden sm:inline">Limpar Cifras</span><span className="sm:hidden">Limpar</span>
                </button>
            </div>
            <div className="flex justify-between items-center pl-1"><p className="text-[8px] md:text-[9px] font-bold text-slate-400 uppercase tracking-widest">Editor Rico</p><p className="text-[8px] md:text-[9px] text-slate-300 italic mr-1">Botão direito na cor para editar atalho</p></div>
        </div>
    );
};
...
const SortableSongItem = ({ song, index, isExpanded, toggleExpand, onUpdate, onTranspose, onDelete, generatingSongId, handleGenerateLyrics, withChords, setWithChords }: any) => {
    ...
    return (
        <div ref={setNodeRef} style={style} className={`bg-white dark:bg-[#0b1221] rounded-[2rem] shadow-sm border border-slate-200 dark:border-white/5 overflow-hidden group mb-4 relative transition-shadow duration-200 ${isDragging ? 'shadow-2xl ring-2 ring-brand-500/50 z-50 scale-[1.03] cursor-grabbing' : ''}`}>
            <div className="p-4 md:p-5 flex items-center gap-3 md:gap-4 select-none bg-white dark:bg-[#0b1221] relative z-10 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors" onClick={toggleExpand}>
                <div className="text-slate-300 group-hover:text-brand-500 transition-colors cursor-grab active:cursor-grabbing p-2 -ml-2 touch-none shrink-0" {...attributes} {...listeners} onClick={(e) => e.stopPropagation()}><i className="fas fa-grip-vertical text-lg"></i></div>
                <div className="w-9 h-9 md:w-10 md:h-10 rounded-xl bg-slate-50 dark:bg-white/5 flex items-center justify-center text-xs md:text-sm font-bold text-slate-500 dark:text-slate-400 shrink-0">{index + 1}</div>
                <div className="flex-1 min-w-0 cursor-pointer">
                    <div className="flex items-center gap-2 mb-0.5">
                        <span className="px-2 py-0.5 rounded-md bg-slate-100 dark:bg-white/10 text-[9px] md:text-[10px] font-bold uppercase text-slate-500 dark:text-slate-300 shrink-0">{song.type}</span>
                        {song.key && <span className="px-2 py-0.5 rounded-md bg-brand-50 dark:bg-brand-900/20 text-[9px] md:text-[10px] font-bold text-brand-600 dark:text-brand-400 shrink-0">{song.key}</span>}
                    </div>
                    <h4 className={`font-bold text-base md:text-lg text-slate-800 dark:text-white truncate ${!song.title && 'italic text-slate-400'}`}>{song.title || 'Nova Música'}</h4>
                </div>
                <button onClick={(e) => {e.stopPropagation(); onDelete()}} className="w-8 h-8 md:w-9 md:h-9 rounded-lg hover:bg-red-50 text-slate-300 hover:text-red-500 transition-colors flex items-center justify-center shrink-0"><i className="fas fa-trash text-sm"></i></button>
                <div className={`w-7 h-7 md:w-8 md:h-8 flex items-center justify-center rounded-full transition-transform duration-300 shrink-0 ${isExpanded ? 'rotate-180 bg-slate-100 dark:bg-white/10 text-slate-600' : 'text-slate-300'}`}><i className="fas fa-chevron-down text-xs"></i></div>
            </div>
            <AnimatePresence>
                {isExpanded && !isDragging && (
                    <motion.div key="content" initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3, ease: "easeInOut" }} className="px-4 md:px-6 pb-8 pt-2 bg-slate-50/50 dark:bg-[#0f172a]/50 border-t border-slate-100 dark:border-white/5 cursor-default overflow-hidden" onClick={e => e.stopPropagation()} onPointerDown={(e) => e.stopPropagation()} onTouchStart={(e) => e.stopPropagation()}>
                         <div className="grid grid-cols-12 gap-4 mb-4">
                             ...
                             <div className="col-span-12 md:col-span-2"><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 block ml-1">Tom</label><div className="relative"><select value={currentRootKey} onChange={e => onTranspose(song.id, e.target.value)} className="w-full pl-4 pr-8 py-3 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 font-bold text-sm outline-none uppercase appearance-none cursor-pointer hover:bg-white/80 transition-colors"><option value="">--</option>{MusicUtils.KEYS.map(k => <option key={k} value={k}>{k}</option>)}</select><i className="fas fa-music absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 text-xs pointer-events-none"></i></div></div>
...
const RepertoryGenerator: React.FC = () => {
  ...
  const handleDuplicate = async (e: React.MouseEvent, rep: Repertory) => {
    e.stopPropagation();
    e.preventDefault();
    
    // if (!confirm(`Deseja duplicar o repertório "${rep.theme}"?`)) return; // Removed confirmation for speed

    try {
        setIsProcessing(true);
        
        // Create a deep copy and sanitize
        const newSongs = (rep.songs || []).map(s => ({
            ...s,
            id: Date.now().toString() + Math.random().toString().slice(2) // New IDs for songs
        }));

        // Remove ID from the original object
        const { id, ...rest } = rep;
        ...
        const newRep: any = {
            ...rest,
            theme: `${rep.theme} (Cópia)`,
            songs: newSongs,
            createdBy: currentUser?.username, // The copier becomes the owner
            createdAt: new Date().toISOString(),
            date: todayStr, // Set to today so it appears at the top/upcoming
            isPrivate: rep.isPrivate // Inherit privacy setting from original
        };

        // Save to Firestore
        await RepertoryService.save(newRep);
        
        if (currentUser) {
            await AuditService.log(currentUser.username, 'Repertory', 'CREATE', `Duplicou repertório: ${rep.theme}`, currentUser.role, currentUser.name);
        }
        
        alert("Repertório duplicado com sucesso! Ele foi definido para a data de hoje.");
  ...
  return (
    <div className="space-y-8 pb-32 animate-fade-in-up w-full overflow-x-hidden">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 relative z-10 mb-8">
        ...
        <div className="bg-slate-100 dark:bg-slate-800 p-1.5 rounded-2xl grid grid-cols-2 lg:flex gap-1 shadow-inner w-full md:w-auto">
          {isAdmin && (
            <button 
              onClick={() => { resetForm(); setActiveTab('build'); }} 
              className={`px-2 md:px-6 py-3 rounded-xl text-[10px] md:text-xs font-bold uppercase tracking-wide transition-all flex items-center justify-center gap-2 ${activeTab === 'build' ? 'bg-white dark:bg-slate-700 text-brand-600 dark:text-white shadow-md' : 'text-slate-500 hover:text-slate-800 dark:text-slate-400'}`}
            >
              <i className="fas fa-plus"></i> <span className="truncate">Criar Novo</span>
            </button>
          )}
          <button 
            onClick={() => setActiveTab('history')} 
            className={`px-2 md:px-6 py-3 rounded-xl text-[10px] md:text-xs font-bold uppercase tracking-wide transition-all flex items-center justify-center gap-2 ${activeTab === 'history' ? 'bg-white dark:bg-slate-700 text-brand-600 dark:text-white shadow-md' : 'text-slate-500 hover:text-slate-800 dark:text-slate-400'}`}
          >
            <i className="fas fa-layer-group"></i> <span className="truncate">Galeria</span>
          </button>
        </div>
      </div>
      ...
      {activeTab === 'history' && (
          <>            <div className="grid grid-cols-3 lg:flex gap-1.5 pb-2 w-full">
            {[
              ...
              <button 
                ...
                className={`px-1 md:px-5 py-2.5 rounded-xl text-[9px] md:text-xs font-bold uppercase tracking-wide transition-all flex items-center justify-center gap-1.5 ${filterStatus === f.id ? 'bg-slate-900 dark:bg-white text-white dark:text-slate-900 shadow-md' : 'text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800'}`}
              >
                <i className={`fas ${f.icon}`}></i> <span className="truncate">{f.label}</span>
              </button>
            ))}
          </div>
<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6">{displayHistory.map((rep) => { const bgImage = rep.coverImage || REPERTORY_COVERS[0]; const isStaticFallback = bgImage === REPERTORY_COVERS[0]; const shouldUsePremium = isStaticFallback || !rep.coverImage; const isDeleting = deletingId === rep.id; const dateObj = new Date(rep.date + 'T12:00:00'); const today = new Date(); today.setHours(0,0,0,0); const repDate = new Date(rep.date + 'T12:00:00'); repDate.setHours(0,0,0,0); const isUpcoming = repDate >= today; const canManage = isSuperAdmin || (currentUser?.role === 'admin' && rep.createdBy === currentUser?.username); const creatorUser = usersList.find(u => u.username === rep.createdBy); const creatorName = creatorUser ? creatorUser.name.split(' ')[0] : (rep.createdBy ? rep.createdBy.split('@')[0] : 'Uziel'); return (<div key={rep.id} className={`group bg-white dark:bg-[#0b1221] rounded-[2.5rem] p-4 shadow-lg hover:shadow-2xl border border-slate-100 dark:border-white/5 transition-all hover:scale-[1.02] relative flex flex-col ${isDeleting ? 'opacity-50 pointer-events-none' : ''} ${isUpcoming ? 'opacity-100' : 'opacity-60 mix-blend-luminosity hover:mix-blend-normal hover:opacity-100 duration-500'}`}><div onClick={() => { const repWithCover = { ...rep, coverImage: bgImage }; setViewingRep(repWithCover); setActiveTab('view'); }} className="cursor-pointer"><div className="aspect-square rounded-[2rem] relative overflow-hidden p-8 flex flex-col justify-between text-white shadow-md z-0 transform-gpu bg-slate-900">{shouldUsePremium ? (<PremiumBackground variant="golden" />) : (<div className="absolute inset-0"><img src={bgImage} onError={(e) => { const target = e.target as HTMLImageElement; target.onerror = null; target.src = REPERTORY_COVERS[0]; }} alt="Capa" className="absolute inset-0 w-full h-full object-cover transition-transform duration-[10s] ease-in-out group-hover:scale-110 opacity-60 rounded-[2rem] animate-breathing" /></div>)}<div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-black/20 rounded-[2rem]"></div><div className="flex justify-between items-start z-10"><span className="text-6xl font-display font-bold text-white drop-shadow-lg">{dateObj.getDate()}</span>{rep.isPrivate && <div className="w-8 h-8 rounded-full bg-black/40 backdrop-blur-md flex items-center justify-center border border-white/20"><i className="fas fa-lock text-xs"></i></div>}</div><div className="absolute bottom-4 right-4 z-10 flex items-center gap-2 bg-black/40 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/10"><div className="w-5 h-5 rounded-full bg-brand-500 flex items-center justify-center text-[10px] font-bold shadow-sm overflow-hidden">{creatorUser?.photoURL ? (<img src={creatorUser.photoURL} alt={creatorName} className="w-full h-full object-cover" />) : (creatorName.charAt(0).toUpperCase())}</div><span className="text-[10px] font-bold text-white uppercase tracking-wider">{creatorName}</span></div></div><div className="mt-5 mb-3 px-2"><span className="block text-[10px] uppercase tracking-[0.2em] font-bold text-slate-400 mb-1">{dateObj.toLocaleDateString('pt-BR', {month:'long', year:'numeric'})}</span><span className="block font-bold text-xl leading-tight line-clamp-2 text-slate-800 dark:text-white group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors">{rep.theme || 'Culto de Domingo'}</span></div></div><div className="mt-auto px-2 pb-2 flex flex-wrap justify-between items-center relative z-20 border-t border-slate-100 dark:border-white/5 pt-4 gap-y-3">
  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2">
    <i className="fas fa-music text-brand-500"></i> {rep.songs.length} faixas
  </span>
  <div className="flex gap-1.5 flex-wrap justify-end">
    <button onClick={(e) => { e.stopPropagation(); setPdfChoiceModal({ isOpen: true, rep }); }} className="w-9 h-9 rounded-xl bg-slate-50 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-500 flex items-center justify-center transition-all active:scale-90 active:bg-red-50 dark:active:bg-red-900/20 group/pdf" title="Exportar PDF">
      <i className="fas fa-file-pdf transition-transform group-active/pdf:scale-75 group-active/pdf:text-red-500 text-sm"></i>
    </button>
    <button onClick={(e) => { e.stopPropagation(); setPptxChoiceModal({ isOpen: true, rep }); }} className="w-9 h-9 rounded-xl bg-slate-50 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-500 flex items-center justify-center transition-all active:scale-90 active:bg-orange-50 dark:active:bg-orange-900/20 group/pptx" title="Exportar PowerPoint">
      <i className="fas fa-file-powerpoint transition-transform group-active/pptx:scale-75 group-active/pptx:text-orange-500 text-sm"></i>
    </button>
    {canManage && (
      <>
        <div className="w-px h-6 bg-slate-200 dark:bg-white/10 mx-1 self-center"></div>
        <button onClick={(e) => handleDuplicate(e, rep)} className="w-9 h-9 rounded-xl bg-slate-50 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-500 hover:text-brand-500 flex items-center justify-center transition-all active:scale-90" title="Duplicar Repertório">
          <i className="fas fa-copy text-sm"></i>
        </button>
        <button onClick={() => loadForEditing(rep)} className="w-9 h-9 rounded-xl bg-brand-50 dark:bg-brand-500/10 hover:bg-brand-100 dark:hover:bg-brand-500/20 text-brand-500 flex items-center justify-center transition-colors" title="Editar">
          <i className="fas fa-pen text-sm"></i>
        </button>
        <button onClick={(e) => requestDelete(e, rep.id!)} disabled={isDeleting} className="w-9 h-9 rounded-xl bg-red-50 dark:bg-red-500/10 hover:bg-red-100 dark:hover:bg-red-500/20 text-red-500 flex items-center justify-center transition-colors disabled:opacity-50" title="Excluir">
          {isDeleting ? <i className="fas fa-circle-notch fa-spin text-xs"></i> : <i className="fas fa-trash text-sm"></i>}
        </button>
      </>
    )}
  </div>
</div></div>); })} {displayHistory.length === 0 && (<div className="col-span-full py-20 text-center opacity-50 flex flex-col items-center"><div className="w-24 h-24 bg-slate-100 dark:bg-white/5 rounded-full flex items-center justify-center mb-4"><i className="fas fa-folder-open text-44 text-slate-300"></i></div><p className="font-bold text-slate-400">Nenhum repertório encontrado.</p></div>)}</div></>
      )}
      {activeTab === 'view' && viewingRep && (
         <div className="space-y-6 md:space-y-10 animate-fade-in-up">
            <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4">
                <button 
                    onClick={() => setActiveTab('history')} 
                    className="flex items-center justify-center gap-2 text-slate-500 hover:text-brand-600 font-bold text-xs uppercase tracking-widest bg-white dark:bg-white/5 px-6 py-4 sm:py-3 rounded-xl shadow-sm transition-colors border border-slate-200 dark:border-white/5"
                >
                    <i className="fas fa-arrow-left"></i> Voltar
                </button>
                <div className="grid grid-cols-2 gap-3 sm:flex sm:gap-2">
                    <button 
                        onClick={() => setPdfChoiceModal({ isOpen: true, rep: viewingRep })} 
                        className="px-4 sm:px-6 py-4 sm:py-3 rounded-xl bg-white dark:bg-white/10 text-slate-800 dark:text-white font-bold hover:bg-slate-100 dark:hover:bg-white/20 transition-all active:scale-95 active:shadow-inner flex items-center justify-center gap-2 shadow-sm text-xs uppercase tracking-wider group"
                    >
                        <i className="fas fa-file-pdf text-red-500 text-lg transition-transform group-active:scale-125 group-active:rotate-12"></i> PDF
                    </button>
                    <button 
                        onClick={() => setPptxChoiceModal({ isOpen: true, rep: viewingRep })} 
                        className="px-4 sm:px-6 py-4 sm:py-3 rounded-xl bg-white dark:bg-white/10 text-slate-800 dark:text-white font-bold hover:bg-slate-100 dark:hover:bg-white/20 transition-all active:scale-95 active:shadow-inner flex items-center justify-center gap-2 shadow-sm text-xs uppercase tracking-wider group"
                    >
                        <i className="fas fa-file-powerpoint text-orange-400 text-lg transition-transform group-active:scale-125 group-active:rotate-12"></i> PPTX
                    </button>
                </div>
            </div>

            <div className="bg-slate-900 rounded-[2.5rem] md:rounded-[4rem] p-8 md:p-20 text-center shadow-2xl relative overflow-hidden text-white group">
                {(() => { 
                    const bgImage = viewingRep.coverImage || REPERTORY_COVERS[0]; 
                    const isStaticFallback = bgImage === REPERTORY_COVERS[0]; 
                    const shouldUsePremium = isStaticFallback || !viewingRep.coverImage; 
                    if (shouldUsePremium) { return <PremiumBackground variant="golden" />; } 
                    return ( <img src={bgImage} onError={(e) => { const target = e.target as HTMLImageElement; target.onerror = null; target.src = REPERTORY_COVERS[0]; }} alt="Capa Hero" className="absolute inset-0 w-full h-full object-cover opacity-60 animate-breathing" /> ); 
                })()}
                <div className="absolute inset-0 bg-gradient-to-t from-slate-900 via-slate-900/60 to-transparent"></div>
                <div className="relative z-10 max-w-4xl mx-auto">
                    <span className="inline-block px-4 py-1.5 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-[9px] md:text-[10px] font-bold uppercase tracking-widest mb-6">Repertório Oficial</span>
                    <h2 className="text-4xl sm:text-5xl md:text-8xl font-display font-bold mb-6 text-shadow-lg leading-[1.1] md:leading-tight">{viewingRep.theme}</h2>
                    <div className="flex flex-col md:flex-row justify-center items-center gap-4 md:gap-8">
                        <p className="text-slate-300 font-medium text-base md:text-xl uppercase tracking-widest flex items-center gap-3">
                            <i className="fas fa-calendar text-brand-400"></i> 
                            {new Date(viewingRep.date + 'T12:00:00').toLocaleDateString('pt-BR', {weekday:'long', day:'numeric', month:'long'})}
                        </p>
                    </div>
                    <div className="mt-8 flex justify-center items-center gap-3 opacity-80">
                        {(() => { 
                            const creatorUser = usersList.find(u => u.username === viewingRep.createdBy); 
                            const creatorName = creatorUser ? creatorUser.name.split(' ')[0] : (viewingRep.createdBy ? viewingRep.createdBy.split('@')[0] : 'Uziel'); 
                            return (
                                <div className="flex items-center gap-2 bg-black/20 backdrop-blur-sm px-4 py-2 rounded-full border border-white/5">
                                    <div className="w-7 h-7 rounded-full bg-brand-500 flex items-center justify-center text-[10px] font-bold overflow-hidden shadow-lg">
                                        {creatorUser?.photoURL ? <img src={creatorUser.photoURL} className="w-full h-full object-cover" /> : creatorName.charAt(0).toUpperCase()}
                                    </div>
                                    <span className="text-[10px] md:text-xs font-bold uppercase tracking-widest">Criado por {creatorName}</span>
                                </div>
                            ); 
                        })()}
                    </div>
            ...
            <div className="space-y-6 max-w-5xl mx-auto">
                {viewingRep.songs.map((song, idx) => { 
                    const spotifyUrl = getSpotifyEmbedUrl(song.link); 
                    const youtubeUrl = getYoutubeEmbedUrl(song.link); 
                    const currentRootKey = song.key ? song.key.replace(/m.*/, '') : ''; 
                    const hasChords = song.lyrics.includes('['); 
                    return (
                        <div key={idx} className="bg-white dark:bg-[#0b1221] rounded-[2.5rem] shadow-sm border border-slate-200 dark:border-white/5 overflow-hidden transition-all duration-300 md:hover:scale-[1.01] hover:shadow-2xl group">
                            <div className="p-6 md:p-10 border-b border-slate-100 dark:border-white/5 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-slate-50/50 dark:bg-white/[0.02]">
                                <div className="flex items-center gap-5 w-full md:w-auto">
                                    <div className="w-14 h-14 md:w-24 md:h-24 rounded-[1.5rem] md:rounded-[2rem] bg-white dark:bg-white/10 flex items-center justify-center text-2xl md:text-4xl font-bold text-slate-300 dark:text-slate-500 shadow-sm border border-slate-100 dark:border-white/5 transition-all duration-300 group-hover:bg-brand-500 group-hover:text-white group-hover:scale-110 shrink-0">
                                        {idx + 1}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <div className="flex flex-wrap items-center gap-2 mb-2">
                                            <span className="px-2 py-1 rounded-md bg-slate-200 dark:bg-white/10 text-[9px] md:text-xs font-bold uppercase text-slate-600 dark:text-slate-300 tracking-wider">
                                                {song.type}
                                            </span>
                                            {hasChords && (
                                                <div className="relative group/key" onClick={e => e.stopPropagation()}>
                                                    <select 
                                                        ...
                                                        onChange={e => handleViewTranspose(song.id, e.target.value)} 
                                                        className="appearance-none bg-brand-100 dark:bg-brand-900/30 text-brand-600 dark:text-brand-400 font-bold text-[9px] md:text-xs py-1 pl-2 md:pl-3 pr-6 md:pr-8 rounded-md cursor-pointer outline-none hover:bg-brand-200 dark:hover:bg-brand-900/50 transition-colors"
                                                    >
                                                        {MusicUtils.KEYS.map(k => (<option key={k} value={k}>{k}</option>))}
                                                    </select>
                                                    <div className="absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none text-brand-600 dark:text-brand-400 text-[8px] md:text-[10px]">
                                                        <i className="fas fa-chevron-down"></i>
                                                    </div>
                                                </div>
                                            )}
                                        </div>
                                        <h3 className="text-2xl md:text-5xl font-bold text-slate-800 dark:text-white leading-tight truncate">{song.title}</h3>
                                        {song.artist && <p className="text-sm md:text-xl font-bold text-slate-400 mt-1 truncate">{song.artist}</p>}
                                    </div>
                                </div>
                                {song.link && !spotifyUrl && !youtubeUrl && (
                                    <a href={song.link} target="_blank" rel="noreferrer" className="w-full md:w-auto px-6 py-3 rounded-xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold text-xs uppercase tracking-wider hover:scale-105 transition-transform flex items-center justify-center gap-2 shadow-lg">
                                        <i className="fas fa-external-link-alt"></i> Ouvir Mídia
                                    </a>
                                )}
                            </div>
                            <div className="flex flex-col">
                                <div className="p-6 md:p-12 w-full overflow-x-auto">
                                    <ChordRenderer text={song.lyrics} center={true} />
                                </div>
                                {(spotifyUrl || youtubeUrl) && (
                                    <div className="bg-slate-50/50 dark:bg-black/20 border-t border-slate-100 dark:border-white/5 w-full p-6 md:p-10">
                                        <div className="max-w-4xl mx-auto">
                                            {spotifyUrl ? (
                                                <div className="w-full rounded-2xl overflow-hidden shadow-lg">
                                                    <iframe src={spotifyUrl} width="100%" height="152" frameBorder="0" allowFullScreen allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy"></iframe>
                                                </div>
                                            ) : (
                                                <div className="w-full aspect-video rounded-2xl overflow-hidden shadow-2xl bg-black">
                                                    <iframe src={youtubeUrl!} width="100%" height="100%" title="YouTube video player" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen className="w-full h-full"></iframe>
                                                </div>
                                            )}
                                        </div>
                                    </div>
